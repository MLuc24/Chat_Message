...
export function MediaPicker({
  ...
  const {
    ...
    uploadType: 'auto', // Auto-detect file type
    serviceUrl: '/api/chat',
    maxFiles,
    maxTotalSizeMB: 100, // 100MB total
    onComplete: (results) => {
    ...
    <div className={`media-picker ${className}`}>
      ...
      {items.length === 0 && (
        ...
        >
          ...
          <span>Pick Media (Max {maxFiles})</span>
        </button>
      )}
            ...
            {items.map((item) => (
              <div key={item.id} className="relative group">
                ...
                <div className="aspect-square rounded-lg overflow-hidden bg-gray-100 relative">
                  ...
                  {/* File Type Badge */}
                  <div className="absolute top-2 left-2 px-2 py-1 bg-black bg-opacity-60 rounded text-white text-xs font-medium uppercase">
                    {item.fileType}
                  </div>
